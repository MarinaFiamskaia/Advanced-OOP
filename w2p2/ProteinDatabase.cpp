...
namespace sdds {
	ProteinDatabase::ProteinDatabase(const char* file) {
		...
		delete[] pd_str; //redundant, not needed
		pd_str = nullptr;
	...
	ProteinDatabase& ProteinDatabase::operator=(ProteinDatabase&& toMove) noexcept {
		if (this != &toMove) {
			delete[] pd_str;
			pd_str = toMove.pd_str;
			toMove.pd_str = nullptr;
			pd_noOfSequences = toMove.pd_noOfSequences;
			toMove.pd_noOfSequences = 0;
		}
		return *this;
	}
	...
	std::string ProteinDatabase::operator[](size_t index)const {
		//need to check for negative index values as invalid
